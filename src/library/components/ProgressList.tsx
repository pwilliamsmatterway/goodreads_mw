...
import { Vertical, ListItem, Spinner, IconBox } from '@matterway/assistant-design-system/content';
...
export function ProgressListItem(p: ProgressListItemProps) {
  ...
  return <FixedListItem title={p.name} description={p.description ?? p.status} left={<Icon status={p.status} />} />;
}
...
export function JobStatusIcon(p: JobStatusIconOptions) {
  return p.status === 'running' ? (
    <Spinner size={32} hasPadding={false} />
  ) : p.status === 'done' ? (
    <IconBox icon="checkmark" boxColor="#E7F1FD" iconColor="#3A95FF" />
  ) : p.status === 'failed' ? (
    <IconBox icon="close" boxColor="#FBECE6" iconColor="#FF5E18" />
  ) : (
