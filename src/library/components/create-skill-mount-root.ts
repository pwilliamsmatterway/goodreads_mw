...
function fixReactForShadowRoot(shadowRoot: ShadowRoot, reactMountRoot: HTMLElement, document: Document) {
...
function addStyles(target: ShadowRoot | HTMLElement) {
  target.append(...Array.from(window.document.head.children).filter(({ tagName }) => tagName === 'STYLE'));
}

export function createSkillMountRoot(options: { identifier: string; onDestroy: (reactMountRoot: HTMLElement) => any }) {
  ...
  const { [assistantTestingModeWindowProperty]: testingMode } = window as {
